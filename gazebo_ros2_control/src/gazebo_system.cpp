  ...
  const hardware_interface::HardwareInfo & hardware_info, gazebo::physics::ModelPtr parent_model)
{
  ...
  this->dataPtr->n_sensors_ =
    this->dataPtr->sim_imu_sensors_.size() + this->dataPtr->sim_ft_sensors_.size();
  ...
  const rclcpp::Time & time, const rclcpp::Duration & period)
{
  ...
  for (const auto & mimic_joint : this->dataPtr->mimic_joints_)
  {
    ...
    if (
      this->dataPtr->joint_control_methods_[mimic_joint.joint_index] & VELOCITY &&
    ...
    if (
      this->dataPtr->joint_control_methods_[mimic_joint.joint_index] & EFFORT &&
